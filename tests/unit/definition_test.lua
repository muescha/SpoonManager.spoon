return function(T)
    ...
    T.test("definition stores raw builder values before resolve", function()
        ...
        local plain = definition.explain()
        T.assertEqual(plain.target.selection_spoon, "A.spoon")
        T.assertEqual(plain.target.name_withName, "B.spoon")
        T.assertFalse(plain.resolved)

        local commanded = definition.command("install").explain()
        T.assertEqual(commanded.target.selection_spoon, "A.spoon")
        T.assertEqual(commanded.target.name_withName, "B.spoon")
